...
"""
class BackboneKeyPoint(nn.Module):
    def __init__(self):
        super(BackboneKeyPoint, self).__init__()

        # if keypoint is voxel type : (batch_size, 63, 5, 13, 13)
...
class UnifiedNetwork(nn.Module):
    ...
    def __init__(self):
        ...
        #model_keypoint = BackboneKeyPoint()
        ...
        # extra layers
        #self.linear = nn.Linear(63 * 5 * 13 * 13, 10816)
    ...
    def forward_extra(self, x, extra):
        ...
        #y = self.features_keypoint(extra)
        extra = extra.view(-1, self.num_flat_features(extra))
