...
from obman_util import handutils
from obman_util.loadutils import fast_load_obj
from obman_util.coordutils import get_coords_2d
...
class ObMan():
    def __init__(self,
        ...
        for side in ['left', 'right']:
            with open('./obman_util/mano_faces_{}.pkl'.format(side), 'rb') as p_f:
    ...
    def load_dataset(self):
        ...
        if os.path.exists(cache_path) and self.use_cache:
            ...
            self.dataset_len = len(annotations['image_names'])
        else:
            idxs = [
                int(imgname.split('.')[0])
